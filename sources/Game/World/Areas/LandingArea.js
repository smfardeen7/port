...
export class LandingArea extends Area
{
    ...
    setLetters()
    {
        ...
        first.getWorldPosition(start)
        last.getWorldPosition(end)
        const mid = start.clone().lerp(end, 0.28)
        mid.y = Math.max(start.y, 0) + 1.35

        const createSign = (lines, width, height, yOffset) =>
        {
